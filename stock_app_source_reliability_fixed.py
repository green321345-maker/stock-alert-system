...
with st.sidebar:
    ...
    st.caption("Finnhub가 401/403/429 또는 업그레이드 응답을 주면 앱 안에 만료/권한/한도 상태를 표시합니다.")
...
def enrich_info_from_statements(ticker_obj, info, fast, hist):
    ...
    price = (
        safe_float((fast or {}).get("lastPrice"))
        or safe_float((fast or {}).get("regularMarketPrice"))
        or safe_float(enriched.get("currentPrice"))
    )
    ...
    if shares is None:
        shares = _pick_statement_value(balance_sheet, ["Ordinary Shares Number", "Share Issued", "Common Stock Shares Outstanding"])

    net_income = _pick_statement_value(income_stmt, ["Net Income", "Net Income Common Stockholders", "NetIncome"])
    revenue = _pick_statement_value(income_stmt, ["Total Revenue", "Operating Revenue", "Revenue"])
    equity = _pick_statement_value(balance_sheet, ["Stockholders Equity", "Common Stock Equity", "Total Equity Gross Minority Interest"])
    ...
    try:
        if enriched.get("revenueGrowth") in [None, "", "None"] and quarterly_income is not None and not quarterly_income.empty:
            for name in ["Total Revenue", "Operating Revenue", "Revenue"]:
                if name in quarterly_income.index:
                    row = quarterly_income.loc[name].dropna()
                    if len(row) >= 2:
    ...
    try:
        if enriched.get("earningsGrowth") in [None, "", "None"] and quarterly_income is not None and not quarterly_income.empty:
...
def source_status(info, hist, yf_target_mean, fh_target_mean, finnhub_status):
    core_keys = ["currentPrice", "marketCap", "trailingEps", "returnOnEquity", "profitMargins"]
    filled = sum(1 for k in core_keys if safe_float(info.get(k)) is not None)
    hist_ok = hist is not None and not hist.empty
    if hist_ok:
        filled += 1
    total = len(core_keys) + 1

    if yf_target_mean is not None:
        filled += 1
    total += 1

    if fh_target_mean is not None:
        filled += 1
    total += 1

    ratio = filled / total if total else 0
    if finnhub_status and "만료" in finnhub_status:
        return "Finnhub 유료/토큰 문제"
    if ratio >= 0.75:
        return "연동 양호"
    if ratio >= 0.45:
        return "일부 제한"
...
def fetch_snapshot(ticker: str, period: str):
    ...
    try:
        ...
        info = enrich_info_from_statements(s, info, fast, hist)
...
def get_yfinance_targets(ticker: str):
    ...
    try:
        ...
        if isinstance(ap, dict):
            mean_v = safe_float(ap.get("mean"))
            high_v = safe_float(ap.get("high"))
            low_v = safe_float(ap.get("low"))
        ...
        mean_v = safe_float(info.get("targetMeanPrice"))
        high_v = safe_float(info.get("targetHighPrice"))
        low_v = safe_float(info.get("targetLowPrice"))
        if mean_v is not None or high_v is not None or low_v is not None:
            return mean_v, high_v, low_v, "Yahoo info 연동"

        price = None
        try:
            price = safe_float(dict(s.fast_info).get("lastPrice"))
        except Exception:
            price = None
        if price is None:
            try:
...
def get_finnhub_targets(ticker: str, api_key: str):
    ...
    try:
        ...
        if status_code in [401, 403]:
            return None, None, None, "Finnhub 유료/토큰 만료 또는 권한 문제"
        if status_code == 429:
            return None, None, None, "Finnhub 호출 한도 초과"
        if "upgrade" in body or "plan" in body or "forbidden" in body:
            return None, None, None, "Finnhub 유료 플랜 종료/권한 부족 가능"
        if status_code == 200:
            return None, None, None, "Finnhub 응답은 왔지만 목표가 데이터 없음"
...
def analyze_ticker(ticker: str, period: str, api_key: str) -> dict:
    ...
    integration_status = source_status(info, hist, yf_mean, fh_mean, fh_status)
    upside = None
    downside = None
