...
def calc_shareholder_yield_direct(info: dict, cashflow: pd.DataFrame):
    market_cap = safe_float(info.get("marketCap"))
    if market_cap is None or market_cap <= 0 or cashflow is None or cashflow.empty:
    ...
    total = dividend_yield + buyback_yield
    outlier = total > 40

    source = "재무제표 직접계산"
    if dividends_paid in [None, 0] and repurchase in [None, 0]:
        source = "데이터 부족"
        total = None
        outlier = False
    elif outlier:
...
def analyze_ticker(ticker: str, period: str, api_key: str) -> dict:
    ...
    fmp_mean, fmp_high, fmp_low, fmp_status, fmp_extra = get_fmp_targets(ticker, fmp_api_key)
    ...
    shareholder_yield, shareholder_yield_source, shareholder_yield_outlier = calc_shareholder_yield_direct(info, raw_statements["cashflow"])
    if shareholder_yield is None:
        div_yield = safe_float(info.get("dividendYield"))
        buyback_yield = safe_float(info.get("buybackYield"))
        if div_yield is not None:
            shareholder_yield = div_yield * 100
            shareholder_yield_source = "API"
            shareholder_yield_outlier = False
        if buyback_yield is not None:
            shareholder_yield = (shareholder_yield or 0) + buyback_yield * 100
            shareholder_yield_source = "API"
            shareholder_yield_outlier = shareholder_yield is not None and shareholder_yield > 40
            if shareholder_yield_outlier:
                shareholder_yield_source = "API (이상치 경고)"
...
with tabs[1]:  # 목표가/해설
    ...
    if current is None:
        ...
        st.caption(f"컨센서스: {current['Consensus']}")
        st.write({
