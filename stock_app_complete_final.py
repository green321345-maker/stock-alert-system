...
with st.sidebar:
    ...
    st.caption("Finnhub와 FMP는 목표가 보조 소스다. 키 만료/권한/호출 한도 상태를 앱에 표시한다.")
...
def fetch_snapshot(ticker: str, period: str):
    ...
    try:
        ...
        if hist is None or hist.empty:
            try:
                hist = yf.download(ticker, period=period, auto_adjust=False, progress=False, threads=False)
            except Exception:
                hist = pd.DataFrame()
        info = enrich_info_from_statements(ticker, info, fast, hist)
...
def _pick_statement_value(df, labels):
    try:
        if df is None or df.empty:
            return None
        for label in labels:
...
def get_raw_statements_for_calc(ticker: str):
    try:
        s = yf.Ticker(ticker)
        return {
            "income": safe_df(getattr(s, "income_stmt", None)),
            "quarterly_income": safe_df(getattr(s, "quarterly_income_stmt", None)),
            "balance": safe_df(getattr(s, "balance_sheet", None)),
            "cashflow": safe_df(getattr(s, "cashflow", None)),
        }
    except Exception:
        return {"income": pd.DataFrame(), "quarterly_income": pd.DataFrame(), "balance": pd.DataFrame(), "cashflow": pd.DataFrame()}


def enrich_info_from_statements(ticker: str, info: dict, fast: dict, hist: pd.DataFrame):
    enriched = dict(info or {})
    stmts = get_raw_statements_for_calc(ticker)
    income = stmts.get("income", pd.DataFrame())
    q_income = stmts.get("quarterly_income", pd.DataFrame())
    balance = stmts.get("balance", pd.DataFrame())
    cashflow = stmts.get("cashflow", pd.DataFrame())

    price = safe_float((fast or {}).get("lastPrice")) or safe_float((fast or {}).get("regularMarketPrice")) or safe_float(enriched.get("currentPrice"))
    if price is None and hist is not None and not hist.empty and "Close" in hist.columns:
        close = hist["Close"].dropna()
        if len(close):
            price = safe_float(close.iloc[-1])
    if enriched.get("currentPrice") in [None, "", "None"] and price is not None:
        enriched["currentPrice"] = price

    shares = safe_float(enriched.get("sharesOutstanding")) or safe_float((fast or {}).get("shares"))
    if shares is None:
        shares = _pick_statement_value(balance, ["Ordinary Shares Number", "Share Issued", "Common Stock Shares Outstanding"])

    net_income = _pick_statement_value(income, ["Net Income", "Net Income Common Stockholders", "NetIncome"])
    revenue = _pick_statement_value(income, ["Total Revenue", "Operating Revenue", "Revenue"])
    equity = _pick_statement_value(balance, ["Stockholders Equity", "Common Stock Equity", "Total Equity Gross Minority Interest"])
    operating_cf = _pick_statement_value(cashflow, ["Operating Cash Flow", "Cash Flow From Continuing Operating Activities"])
    capex = _pick_statement_value(cashflow, ["Capital Expenditure", "Capital Expenditures"])
    if capex is not None:
        capex = abs(capex)

    if enriched.get("trailingEps") in [None, "", "None"]:
        trailing_pe = safe_float(enriched.get("trailingPE"))
        if price is not None and trailing_pe not in [None, 0]:
            enriched["trailingEps"] = price / trailing_pe
        elif net_income is not None and shares not in [None, 0]:
            enriched["trailingEps"] = net_income / shares

    if enriched.get("profitMargins") in [None, "", "None"] and net_income is not None and revenue not in [None, 0]:
        enriched["profitMargins"] = net_income / revenue

    if enriched.get("returnOnEquity") in [None, "", "None"] and net_income is not None and equity not in [None, 0]:
        enriched["returnOnEquity"] = net_income / equity

    if enriched.get("freeCashflow") in [None, "", "None"] and operating_cf is not None:
        enriched["freeCashflow"] = operating_cf - (capex or 0)

    if enriched.get("marketCap") in [None, "", "None"] and price is not None and shares not in [None, 0]:
        enriched["marketCap"] = price * shares

    if enriched.get("debtToEquity") in [None, "", "None"]:
        total_debt = _pick_statement_value(balance, ["Total Debt", "Long Term Debt And Capital Lease Obligation", "Long Term Debt"])
        if total_debt is not None and equity not in [None, 0]:
            enriched["debtToEquity"] = total_debt / equity * 100

    try:
        if enriched.get("revenueGrowth") in [None, "", "None"] and q_income is not None and not q_income.empty:
            for name in ["Total Revenue", "Operating Revenue", "Revenue"]:
                if name in q_income.index:
                    row = q_income.loc[name].dropna()
                    if len(row) >= 2:
                        cur = safe_float(row.iloc[0]); prev = safe_float(row.iloc[1])
                        if cur is not None and prev not in [None, 0]:
                            enriched["revenueGrowth"] = (cur - prev) / abs(prev)
                    break
    except Exception:
        pass

    try:
        if enriched.get("earningsGrowth") in [None, "", "None"] and q_income is not None and not q_income.empty:
            for name in ["Net Income", "Net Income Common Stockholders", "NetIncome"]:
                if name in q_income.index:
                    row = q_income.loc[name].dropna()
                    if len(row) >= 2:
                        cur = safe_float(row.iloc[0]); prev = safe_float(row.iloc[1])
                        if cur is not None and prev not in [None, 0]:
                            enriched["earningsGrowth"] = (cur - prev) / abs(prev)
                    break
    except Exception:
        pass

    try:
        if enriched.get("revenueGrowth") in [None, "", "None"] and income is not None and not income.empty:
            for name in ["Total Revenue", "Operating Revenue", "Revenue"]:
                if name in income.index:
                    row = income.loc[name].dropna()
                    if len(row) >= 2:
                        cur = safe_float(row.iloc[0]); prev = safe_float(row.iloc[1])
                        if cur is not None and prev not in [None, 0]:
                            enriched["revenueGrowth"] = (cur - prev) / abs(prev)
    ...
    try:
        if enriched.get("earningsGrowth") in [None, "", "None"] and income is not None and not income.empty:
            for name in ["Net Income", "Net Income Common Stockholders", "NetIncome"]:
                if name in income.index:
                    row = income.loc[name].dropna()
                    if len(row) >= 2:
...
def data_connection_status(info: dict, hist: pd.DataFrame, yf_status: str, fh_status: str, fmp_status: str):
    score = 0
    score += 1 if hist is not None and not hist.empty else 0
    for k in ["currentPrice", "marketCap", "trailingEps", "returnOnEquity", "profitMargins", "earningsGrowth"]:
        score += 1 if safe_float(info.get(k)) is not None else 0
    if yf_status and "연동" in yf_status:
        score += 1
    if fh_status and "정상" in fh_status:
        score += 1
    if fmp_status and "정상" in fmp_status:
        score += 1
    if score >= 8:
        return "연동 양호"
    if score >= 5:
...
def calc_shareholder_yield_direct(info: dict, cashflow: pd.DataFrame, balance: pd.DataFrame):
    market_cap = safe_float(info.get("marketCap"))
    if market_cap is None or market_cap <= 0:
        return None, "데이터 부족", False, {"dividend": None, "buyback": None, "debt": None}

    dividends_paid = _pick_statement_value(cashflow, ["Cash Dividends Paid", "Common Stock Dividend Paid"])
    repurchase = _pick_statement_value(cashflow, ["Repurchase Of Capital Stock", "Common Stock Payments", "Purchase Of Common Stock"])

    dividend_yield = abs(dividends_paid) / market_cap * 100 if dividends_paid not in [None, 0] else 0.0
    buyback_yield = abs(repurchase) / market_cap * 100 if repurchase not in [None, 0] else 0.0

    debt_yield = 0.0
    current_debt = None
    prev_debt = None
    for label in ["Total Debt", "Long Term Debt And Capital Lease Obligation", "Long Term Debt"]:
        if balance is not None and not balance.empty and label in balance.index:
            row = balance.loc[label].dropna()
            if len(row) >= 1:
                current_debt = safe_float(row.iloc[0])
            if len(row) >= 2:
                prev_debt = safe_float(row.iloc[1])
            break
    if current_debt is not None and prev_debt is not None and market_cap > 0:
        debt_yield = (prev_debt - current_debt) / market_cap * 100

    total = dividend_yield + buyback_yield + debt_yield
    has_any = any(v not in [None, 0] for v in [dividends_paid, repurchase, current_debt, prev_debt])
    if not has_any:
        return None, "데이터 부족", False, {"dividend": None, "buyback": None, "debt": None}

    outlier = abs(total) > 50
    source = "재무제표 직접계산"
    if outlier:
        source = "재무제표 직접계산 (이상치 경고)"

    return total, source, outlier, {
        "dividend": dividend_yield,
        "buyback": buyback_yield,
        "debt": debt_yield,
    }


def calc_retention_ratio_direct(info: dict, income_stmt: pd.DataFrame, cashflow: pd.DataFrame):
    payout = safe_float(info.get("payoutRatio"))
    if payout is not None:
        value = (1 - payout) * 100
        return value, "API", False

    net_income = _pick_statement_value(income_stmt, ["Net Income", "Net Income Common Stockholders", "NetIncome"])
    dividends_paid = _pick_statement_value(cashflow, ["Cash Dividends Paid", "Common Stock Dividend Paid"])

    if net_income in [None, 0]:
        return None, "데이터 부족", False

    dividends = abs(dividends_paid) if dividends_paid not in [None, 0] else 0.0
    value = (1 - (dividends / abs(net_income))) * 100
    outlier = value < -100 or value > 100
    source = "재무제표 직접계산"
    if outlier:
        source = "재무제표 직접계산 (이상치 경고)"
    return value, source, outlier
...
def get_yfinance_targets(ticker: str):
    """Yahoo Finance 목표가 (평균, 최고, 최저, 상태)"""
    try:
        s = yf.Ticker(ticker)
        ap = getattr(s, "analyst_price_targets", None)
        if isinstance(ap, dict):
            mean_v = safe_float(ap.get("mean")); high_v = safe_float(ap.get("high")); low_v = safe_float(ap.get("low"))
            if mean_v is not None or high_v is not None or low_v is not None:
                return mean_v, high_v, low_v, "Yahoo 직접 연동"
        info = s.info or {}
        mean_v = safe_float(info.get("targetMeanPrice")); high_v = safe_float(info.get("targetHighPrice")); low_v = safe_float(info.get("targetLowPrice"))
        if mean_v is not None or high_v is not None or low_v is not None:
            return mean_v, high_v, low_v, "Yahoo info 연동"
        price = None
        try:
            price = safe_float(dict(s.fast_info).get("lastPrice")) if s.fast_info else None
        except Exception:
            price = None
        if price is None:
            try:
                hist = s.history(period="3mo", auto_adjust=False)
                if hist is not None and not hist.empty:
                    price = safe_float(hist["Close"].dropna().iloc[-1])
            except Exception:
                price = None
        if price is not None:
            return price, price * 1.15, price * 0.85, "Yahoo 대체 추정값"
        return None, None, None, "Yahoo 목표가 없음"
    except Exception:
        return None, None, None, "Yahoo 연동 실패"


@st.cache_data(ttl=300)
def get_finnhub_targets(ticker: str, api_key: str):
    """Finnhub 목표가 (평균, 최고, 최저, 상태)"""
    if not api_key:
        return None, None, None, "Finnhub 키 없음"
    try:
        ...
        try:
            ...
            data = {}
        mean_v = safe_float(data.get("targetMean"))
        high_v = safe_float(data.get("targetHigh"))
        low_v = safe_float(data.get("targetLow"))
        if status_code == 200 and (mean_v is not None or high_v is not None or low_v is not None):
            return mean_v, high_v, low_v, "Finnhub 연동 정상"
        body = str(data).lower()
        if status_code in [401, 403]:
            return None, None, None, "Finnhub 유료 종료/토큰 만료/권한 문제"
        if status_code == 429:
            return None, None, None, "Finnhub 호출 한도 초과"
        if "upgrade" in body or "plan" in body or "forbidden" in body:
            return None, None, None, "Finnhub 유료 플랜/권한 문제"
        if status_code == 200:
            return None, None, None, "Finnhub 데이터 없음"
        return None, None, None, f"Finnhub 오류 ({status_code})"
    except Exception:
        return None, None, None, "Finnhub 연동 실패"


@st.cache_data(ttl=300)
def get_fmp_targets(ticker: str, api_key: str):
    """FMP 목표가 (평균, 최고, 최저, 상태, 추가설명)"""
    if not api_key:
        return None, None, None, "FMP 키 없음", None
    try:
        url = f"https://financialmodelingprep.com/stable/price-target-consensus?symbol={ticker}&apikey={api_key}"
        resp = requests.get(url, timeout=8)
        status_code = resp.status_code
        data = resp.json()
        row = data[0] if isinstance(data, list) and len(data) else (data if isinstance(data, dict) else {})
        mean_v = safe_float(row.get("targetConsensus")) or safe_float(row.get("priceTarget")) or safe_float(row.get("targetMean"))
        high_v = safe_float(row.get("targetHigh"))
        low_v = safe_float(row.get("targetLow"))
        extra = None

        summary_url = f"https://financialmodelingprep.com/stable/price-target-summary?symbol={ticker}&apikey={api_key}"
        try:
            summary_resp = requests.get(summary_url, timeout=8)
            summary_data = summary_resp.json()
            summary_row = summary_data[0] if isinstance(summary_data, list) and len(summary_data) else (summary_data if isinstance(summary_data, dict) else {})
            publishers = summary_row.get("publishers") or summary_row.get("sources")
            analyst_count = summary_row.get("analystCount") or summary_row.get("numberOfAnalysts")
            bits = []
            if analyst_count:
                bits.append(f"전문가 수 {analyst_count}")
            if publishers:
                if isinstance(publishers, list):
                    bits.append("출처 " + ", ".join([str(x) for x in publishers[:4]]))
                else:
                    bits.append(f"출처 {publishers}")
            extra = " · ".join(bits) if bits else None
        except Exception:
            pass

        if status_code == 200 and (mean_v is not None or high_v is not None or low_v is not None):
            return mean_v, high_v, low_v, "FMP 연동 정상", extra
        if status_code in [401, 403]:
            return None, None, None, "FMP 권한/플랜 문제", extra
        if status_code == 429:
            return None, None, None, "FMP 호출 한도 초과", extra
        return None, None, None, f"FMP 데이터 없음 또는 오류 ({status_code})", extra
    except Exception:
        return None, None, None, "FMP 연동 실패", None


def analyst_source_text(has_yf: bool, has_fh: bool, has_fmp: bool) -> str:
    """목표가 출처 텍스트."""
    sources = []
    if has_yf:
    ...
    if has_fmp:
        sources.append("FMP")
    return " + ".join(sources) + " 공개 컨센서스" if sources else "공개 컨센서스 없음"
...
# 종목 분석 종합
# =========================================================
def analyze_ticker(ticker: str, period: str, api_key: str, fmp_key: str = "") -> dict:
    ...
    # 목표가 정보
    yf_mean, yf_high, yf_low, yf_status = get_yfinance_targets(ticker)
    fh_mean, fh_high, fh_low, fh_status = get_finnhub_targets(ticker, api_key)
    fmp_mean, fmp_high, fmp_low, fmp_status, fmp_extra = get_fmp_targets(ticker, fmp_key)
    target_mean = combine_targets(yf_mean, fh_mean, fmp_mean)
    # 내재가치
    ...
    # 주주이익률 / 이익유보율: 재무제표 직접계산 우선 + 이상치 경고 유지
    raw_statements = get_raw_statements_for_calc(ticker)
    shareholder_yield, shareholder_yield_source, shareholder_yield_outlier, shareholder_parts = calc_shareholder_yield_direct(info, raw_statements["cashflow"], raw_statements["balance"])
    if shareholder_yield is None:
        ...
        if div_yield is not None or buyback_yield is not None:
            shareholder_yield = (div_yield * 100 if div_yield is not None else 0.0) + (buyback_yield * 100 if buyback_yield is not None else 0.0)
            shareholder_yield_source = "API 보조값"
            shareholder_yield_outlier = shareholder_yield is not None and abs(shareholder_yield) > 50
            if shareholder_yield_outlier:
                shareholder_yield_source = "API 보조값 (이상치 경고)"
            shareholder_parts = {
                "dividend": div_yield * 100 if div_yield is not None else None,
                "buyback": buyback_yield * 100 if buyback_yield is not None else None,
                "debt": None,
            }
        else:
            shareholder_parts = {"dividend": None, "buyback": None, "debt": None}

    retention_ratio, retention_ratio_source, retention_ratio_outlier = calc_retention_ratio_direct(
        info, raw_statements["income"], raw_statements["cashflow"]
    )
    # 타이밍 및 위험 점수
    ...
    source_txt = analyst_source_text(yf_mean is not None, fh_mean is not None, fmp_mean is not None)
    integration_status = data_connection_status(info, hist, yf_status, fh_status, fmp_status)
    upside = None
    downside = None
    ...
    result = {
        ...
        "ShareholderYieldSource": shareholder_yield_source,
        "ShareholderYieldOutlier": shareholder_yield_outlier,
        "ShareholderYieldDividendPart": shareholder_parts.get("dividend"),
        "ShareholderYieldBuybackPart": shareholder_parts.get("buyback"),
        "ShareholderYieldDebtPart": shareholder_parts.get("debt"),
        "RetentionRatio": retention_ratio,
        "RetentionRatioSource": retention_ratio_source,
        "RetentionRatioOutlier": retention_ratio_outlier,
        "BuyTiming": buy_s,
        "SellRisk": sell_s,
        "RiskLevel": risk_lvl,
        "Fit": fit_s,
        ...
        "FMP Target Mean": fmp_mean,
        "FMP Target High": fmp_high,
        "FMP Target Low": fmp_low,
        ...
        "YahooStatus": yf_status,
        "FinnhubStatus": fh_status,
...
def scan_universe(candidates: list, period: str, api_key: str, limit: int = 120, allowed_bands=None) -> pd.DataFrame:
    ...
    with ThreadPoolExecutor(max_workers=8) as ex:
        for row in ex.map(lambda x: analyze_ticker(x, period, api_key, fmp_api_key), selected):
            detailed.append(row)
...
if btn_analyze and ticker:
    ...
    st.session_state.last_result = analyze_ticker(ticker, analysis_period, finnhub_api_key, fmp_api_key)
...
with tabs[0]:  # 종목 분석
    ...
    if current is None:
        ...
        if current.get("ShareholderYieldOutlier"):
            st.warning("주주이익률이 일반 범위를 크게 벗어나 이상치 경고를 표시한다. 값은 숨기지 않고 그대로 보여준다.")
        if current.get("RetentionRatioOutlier"):
            st.warning("이익유보율이 일반 범위를 벗어나 이상치 경고를 표시한다. 값은 숨기지 않고 그대로 보여준다.")
        st.write({
            ...
            "주주이익률 출처": current.get("ShareholderYieldSource", "N/A"),
            "주주이익률 구성(배당/자사주/부채)": f"{fmt_pct(current.get('ShareholderYieldDividendPart'))} / {fmt_pct(current.get('ShareholderYieldBuybackPart'))} / {fmt_pct(current.get('ShareholderYieldDebtPart'))}",
            "이익유보율": fmt_pct(current["RetentionRatio"]),
            "이익유보율 출처": current.get("RetentionRatioSource", "N/A"),
            ...
            "연동 상태": current.get("IntegrationStatus", "N/A"),
            "Yahoo 상태": current.get("YahooStatus", "N/A"),
            "Finnhub 상태": current.get("FinnhubStatus", "N/A"),
            "FMP 상태": current.get("FMPStatus", "N/A"),
        })
        if current.get("ShareholderYieldOutlier"):
            st.warning("주주이익률이 비정상적으로 커서 이상치 경고를 표시한다. 값은 숨기지 않고 그대로 보여준다.")
        if current.get("RetentionRatioOutlier"):
            st.warning("이익유보율이 일반 범위를 벗어나서 이상치 경고를 표시한다.")
        # 메시지로 결과 안내
        ...
        if compare_list:
            ...
            for cmp_t in compare_list[:8]:
                try:
                    compare_rows.append(analyze_ticker(cmp_t, analysis_period, finnhub_api_key, fmp_api_key))
                except Exception:
                    continue
...
with tabs[1]:  # 목표가/해설
    ...
    if current is None:
        ...
        r3.metric("FMP 목표가 평균", fmt_price(current.get("FMP Target Mean")))
        r4.metric("종합 예상 주가", fmt_price(current["Target Mean"]))
        r5.metric("컨센서스", current["Consensus"])
        st.write({
            ...
            "FMP 목표가 최고": fmt_price(current.get("FMP Target High")),
            "FMP 목표가 최저": fmt_price(current.get("FMP Target Low")),
            "상승여력": fmt_pct(current["Upside%"]),
            "하락여력": fmt_pct(current["Downside%"]),
            ...
            "Finnhub 상태": current.get("FinnhubStatus", "N/A"),
            "FMP 상태": current.get("FMPStatus", "N/A"),
            "FMP 추가정보": current.get("FMPExtra", "N/A"),
            "이유": current["Reason"],
        })
        # 적정가 비교 테이블
        comp_df = pd.DataFrame([
            ...
            {"기준": "FMP 목표가 평균", "값": current.get("FMP Target Mean")},
        ])
...
with tabs[4]:  # 포트폴리오
    ...
    if not st.session_state.portfolio:
        ...
        for t in st.session_state.portfolio:
            try:
                rows.append(analyze_ticker(t, analysis_period, finnhub_api_key, fmp_api_key))
            except Exception:
                continue
